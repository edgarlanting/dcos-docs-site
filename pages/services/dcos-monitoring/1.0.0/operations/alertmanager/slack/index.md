...
Create the secret `slackapiurl` for the Slack API URL ([Webhook URL](https://api.slack.com/incoming-webhooks)) to use for notifications.

```bash
dcos security secrets create --value=<SLACK_API_URL> slackapiurl
```

When installing the service, you must configure the `secrets` section to point to the secret `slackapiurl` created.
